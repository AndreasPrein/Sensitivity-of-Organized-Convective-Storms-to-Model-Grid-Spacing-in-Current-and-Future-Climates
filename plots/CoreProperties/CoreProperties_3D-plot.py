#!/usr/bin/env python
'''
    File name: CoreProperties_3D-plot.py
...
rgsColors=['k','#e31a1c']
rgsPeriods=['CUR','FUT']
rgsStatsSel=['Wwidth','Wdepth','Wvolume']
...
for ud in range(2):
    for st in range(len(rgsStatsSel)):
        ...
        for pe in range(2):
            ...
            # # fit exponential function to estimate the asymptote --> convergence
            # if rgsStatsSel[st] != 'Wdepth':
            #     try:
            #         popt, pcov = curve_fit(func, rgrDX[2:], DATA_pe[2:,50], p0=(1, 1e-6, 1))
            #     except:
            #         stop()
            #     xx = Xaxis*1000.
            #     yy = func(xx, *popt)
            #     plt.plot(xx/1000.,yy, ls='--', color=col)
            #     stop()
        ...
        ax.text(0.03,1.03, rgsLableABC[st+ud*3]+') ', ha='left',va='bottom', \
...
# Plot the figure
sPlotFile=sPlotDir
sPlotName= 'MCS-3D-Draft-Geometry.pdf'
