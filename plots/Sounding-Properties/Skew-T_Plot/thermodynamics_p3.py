from numpy import interp,pi,cos,sin,arctan,sqrt,linspace,min,exp,log,where,squeeze
...
Lv=2.4883e6              # Latent Heat of vaporisation
...
def TempToDewpTemp(mr,pres):
    """Calculate Dewpoint Temperature from Temp., Mixing Ratio, and
    Pres. We simplify this and do not distiquish between water and ice. The
    introduced error seems fairly small
    ...
    Tref=293.15
    esref=2339
    DPtemp=1/(1/Tref-(Rs_v/Lv)*log(((pres*mr)/((Rs_da/Rs_v)+mr))/esref))
...
def DTtoRH(DT,T):
    ...
    B1=243.04
    A1=17.625
    RH=100*exp(((A1*B1)*(DT-T))/((B1+DT)*(B1+T)))
    return RH
...
def RHtoDT(RH,T):
    ...
    B1=243.04
    A1=17.625
    TD=(B1*(log(RH/100)+(A1*T)/(B1+T)))/(A1-log(RH/100)-(A1*T)/(B1+T))
    return TD
