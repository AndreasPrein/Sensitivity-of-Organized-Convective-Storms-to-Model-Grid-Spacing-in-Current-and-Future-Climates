...
class Sounding(UserDict):
    ...
    def get_cape(self, startp, startt, startdp, totalcape=False):
        ...
        # ADDITION --> calculate cumulative CAPE
        cCAPE=scipy.integrate.cumtrapz(9.81*(tparcel[cond1]-tempenv[cond1])/(tempenv[cond1] + 273.15),hghtenv[cond1])
        rgiTrues=np.where(cond1 == 1)[0]
        if CAPE != 0:
            cCAPE=np.append(cCAPE,[cCAPE[-1]]*((hghtenv.shape-rgiTrues[-1])[0]))
            cCAPE=np.append([0]*((rgiTrues[0])),cCAPE)
        else:
            cCAPE=np.nan
        ...
        return P_lcl, P_lfc, P_el, CAPE, CIN, cCAPE, hghtenv
